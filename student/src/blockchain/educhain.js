...
export async function buyCourseOnChain({ courseId, price }) {
  ...
  const tx = await contract.buyCourse(BigInt(courseIdNumber), {
    value: parseEther(String(priceNumber)),
  });
...
export async function verifyCertificateHashOnChain({
  ...
  if (!studentWallet || !onChainCourseId || !certificateHash) {
    return {
      checked: false,
      matched: false,
      onChainHash: '',
      message: 'Thiếu dữ liệu để kiểm tra on-chain',
    };
  }
  ...
  if (!isBlockchainConfigured()) {
    return {
      ...
      matched: false,
      onChainHash: '',
      message: 'Chưa cấu hình contract address',
    };
  }
  ...
  const onChainHash = await contract.certificateHashes(
    getAddress(studentWallet),
    BigInt(Number(onChainCourseId))
  );

  const matched =
    String(onChainHash || '').trim() === String(certificateHash || '').trim();
  ...
  return {
    ...
    onChainHash,
    message: matched
      ? 'Certificate hash khớp với dữ liệu on-chain'
      : 'Certificate hash không khớp với dữ liệu on-chain',
  };
}
...
export async function getCertificateHashOnChain({
  studentWallet,
  onChainCourseId,
}) {
  if (!studentWallet || !onChainCourseId) {
    return {
      checked: false,
      exists: false,
      onChainHash: '',
      message: 'Thiếu studentWallet hoặc onChainCourseId',
    };
  }
  ...
  if (!isBlockchainConfigured()) {
    return {
      ...
      exists: false,
      onChainHash: '',
      message: 'Chưa cấu hình VITE_CONTRACT_ADDRESS',
    };
  }
  ...
  return {
    checked: true,
    exists: Boolean(String(onChainHash || '').trim()),
    onChainHash,
    message: onChainHash
      ? 'Tìm thấy certificate hash trên blockchain'
      : 'Không tìm thấy certificate hash trên blockchain',
  };
}

export async function findCertificateIssuedEventByHash(certificateHash) {
  if (!certificateHash || !String(certificateHash).trim()) {
    return {
      found: false,
      message: 'Thiếu certificateHash để kiểm tra blockchain',
    };
  }
  ...
  const inputHash = String(certificateHash).trim().toLowerCase();

  for (const event of events) {
    const eventHash = String(event.args.certHash || '').trim().toLowerCase();

    if (eventHash === inputHash) {
      const receipt = await provider.getTransactionReceipt(event.transactionHash);
      const tx = await provider.getTransaction(event.transactionHash);
      ...
      return {
        ...
        time: block?.timestamp
          ? new Date(block.timestamp * 1000).toLocaleString('vi-VN')
          : '',
        issuerWallet: tx?.from || '',
        contractAddress,
        studentWallet: event.args.student,
        onChainCourseId: String(event.args.courseId),
        certificateHash: event.args.certHash,
        message: 'Tìm thấy chứng chỉ trên blockchain qua event CertificateIssued',
      };
    }
  }
  ...
  return {
    found: false,
    source: 'blockchain',
    message: 'Không tìm thấy certificateHash này trên blockchain',
  };
}